...
Let's combine the data from the previous two plots into a single plot where we
will also be able to see the distribution of age per category.

```{r Athlete_Age_by_Categories}
ggplot(data = subset(unique_athletes, !is.na(gender)),
       aes(x = age)) +
...
```{r Scores_by_Workout_and_Division}
...
```{r message=FALSE, warning=FALSE, '15_1_Scores'}
...
```{r message=FALSE, warning=FALSE, '15_1_Scores_by_Deadlift'}
ggplot(open.15.1.rx,
...
```{r message=FALSE, warning=FALSE, '15_1_Scores_by_Snatch'}
...
```{r message=FALSE, warning=FALSE, '15_1a_Scores'}
...
relation to the score. Let's make a plot of the average result by weight and
find out.

```{r message=FALSE, warning=FALSE, '15_1a_Scores_by_Weight'}
...
```{r message=FALSE, warning=FALSE, '15_2_Scores'}
...
```{r message=FALSE, warning=FALSE, '15_2_Female_Scores_Zoom'}
...
```{r message=FALSE, warning=FALSE, '15_2_Mean_Scores_by_Pullup'}
...
```{r message=FALSE, warning=FALSE, '15_3_Scores'}
...
```{r message=FALSE, warning=FALSE, '15_3_Female_Scores'}
...
```{r message=FALSE, warning=FALSE, '15_4_Mean_Scores_by_Weight'}
...
```{r message=FALSE, warning=FALSE, '15_4_Mean_Scores_by_Height'}
ggplot(open.15.4.rx,
...
```{r message=FALSE, warning=FALSE, '15_4_Male_Scores_by_Weight_and_Height'}
...
```{r message=FALSE, warning=FALSE, '15_4_Female_Scores_by_Weight_and_Height'}
ggplot(subset(open.15.4.rx,
...
```{r message=FALSE, warning=FALSE, '15_5_Score_by_Age'}
open.15.5 <- subset(cf, stage == 5 & category == 'Rx')

ggplot(open.15.5,
       aes(x = age, y = score)) +
  geom_jitter(alpha = 1/40, aes(color = division)) +
  geom_smooth() +
  scale_color_brewer(palette = 'Set1') +
...
```{r message=FALSE, warning=FALSE, '15_5_Mean_Score_by_Age'}
ggplot(open.15.5,
       aes(x = age, y = score)) +
  scale_x_continuous(limits = c(18, 55),
                     breaks = seq(15, 60, 5)) +
...
```{r message=FALSE, warning=FALSE, Mean_Scores_by_Age}
...
```{r message=FALSE, warning=FALSE, Create_Overall_Rank}
...
```{r message=FALSE, warning=FALSE, Overall_Rank_by_Age}
ggplot(subset(cf, category == 'Rx'),
...
ggplot(subset(cf, category == 'Rx'),
  ...
  facet_wrap(~stage,
             ...
             scales = 'free') +
  labs(x = 'Age (Years)',
       y = 'Mean Score (Number of Repetitions or Seconds)',
       color = 'Division',
       title = 'Mean Scores by Age for Each Workout')
```
